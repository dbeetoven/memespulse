...
const getCarbonswapMemesData = (smudge, supply, imgURl) => {
  return {
    ...
    market_cap_change_24h:
      Number(smudge[0]?.priceUSD) * 100000 -
      Number(smudge[1]?.priceUSD) * 100000,
    image: imgURl,
...
export default function CryptoList({ tokens }) {
  ...
  if (data) {
    _tokens = [
      getCarbonswapMemesData(data.tokenDayDatas, 100000, '/smudge.jpg'),
      ...tokens,
    ]
  }
        ...
        <ul className="responsive-table">
          ...
          {_tokens.map((token) => (
            <li
              className="table-row"
              key={token.id}
              onClick={() => window.open(token.url)}
            >
              <div className="col col-0 text-center" data-label="">
                <img
                  className="avatar"
                  src={token.image}
                  alt={token.name}
              ...
              <div
                className="col col-1 text-center  text-capitalize"
                data-label="Token: "
              >
                {token.name}
              </div>
              <div className="col col-2 text-center " data-label="Symbol: ">
                {token.symbol}
              </div>
              <div
                className={`col col-3 text-center ${
